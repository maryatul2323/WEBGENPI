<div>
    <div class="w-full lg:max-w-screen-md mx-auto">
        <div class="bg-white rounded-xl shadow-md p-6 md:p-10 lg:p-12">
            <form
                wire:submit.prevent="submit"
                class="space-y-16"
            >
                <div>
                    <h1 class="text-black font-bold text-xl capitalize">
                        data travel
                    </h1>
                    <div class="mt-8 space-y-8">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-8">
                            <div class="col-span-1">
                                <label 
                                    for="title"
                                    class="block mb-1 text-black font-medium capitalize"
                                >
                                    judul
                                </label>
                                <p class="text-sm text-true-gray-700">
                                    Tambahkan judul travel baru dengan singkat dan jelas
                                </p>
                            </div>
                            <div class="col-span-1 pt-0 md:pt-4">
                                <input 
                                    type="text" 
                                    wire:model="title"
                                    id="title"
                                    class="w-full rounded-lg border-black/75 focus:outline-none focus:ring focus:ring-black/20 focus:border-black"
                                >
                                @error('title')
                                    <span class="block mt-2 text-xs text-red-600">
                                        {{ $message }}
                                    </span>
                                @enderror
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-8">
                            <div class="col-span-1">
                                <label 
                                    for="description"
                                    class="block mb-1 text-black font-medium capitalize"
                                >
                                    deskripsi
                                </label>
                                <p class="text-sm text-true-gray-700">
                                    Tambahkan deskripsi gambaran mengenai travel yang dimaksud
                                </p>
                            </div>
                            <div class="col-span-1 pt-0 md:pt-4">
                                <textarea 
                                    wire:model="description"
                                    id="description"
                                    class="w-full min-h-[150px] border-black/75 rounded-lg focus:outline-none focus:ring focus:ring-black/20 focus:border-black"
                                ></textarea>
                                @error('description')
                                    <span class="block mt-0 text-xs text-red-600">
                                        {{ $message }}
                                    </span>
                                @enderror
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-8">
                            <div class="col-span-1">
                                <label 
                                    for="price-starting-from"
                                    class="block mb-1 text-black font-medium capitalize"
                                >
                                    harga mulai
                                </label>
                                <p class="text-sm text-true-gray-700">
                                    Tambahkan harga travel mulai dari yang paling murah
                                </p>
                            </div>
                            <div class="col-span-1 pt-0 md:pt-4">
                                <div class="flex items-center space-x-2">
                                    <span
                                        class="flex-shrink-0 px-3 py-2 rounded-lg border border-black"
                                    >
                                        Rp
                                    </span>
                                    <input 
                                        type="number" 
                                        wire:model="priceStartingFrom"
                                        id="price-starting-from"
                                        class="w-full rounded-lg border-black/75 focus:outline-none focus:ring focus:ring-black/20 focus:border-black"
                                    >
                                </div>
                                @error('priceStartingFrom')
                                    <span class="block mt-2 text-xs text-red-600">
                                        {{ $message }}
                                    </span>
                                @enderror
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-8">
                            <div class="col-span-1">
                                <label 
                                    for="service-duration"
                                    class="block mb-1 text-black font-medium capitalize"
                                >
                                    durasi layanan
                                </label>
                                <p class="text-sm text-true-gray-700">
                                    Tambahkan lama durasi layanan yang akan diberikan, misalkan "12 jam"
                                </p>
                            </div>
                            <div class="col-span-1 pt-0 md:pt-4">
                                <input 
                                    type="text" 
                                    wire:model="serviceDuration"
                                    id="service-duration"
                                    class="w-full rounded-lg border-black/75 focus:outline-none focus:ring focus:ring-black/20 focus:border-black"
                                >
                                @error('serviceDuration')
                                    <span class="block mt-2 text-xs text-red-600">
                                        {{ $message }}
                                    </span>
                                @enderror
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-8">
                            <div class="col-span-1">
                                <label 
                                    class="block mb-1 text-black font-medium capitalize"
                                >
                                    gambar utama
                                </label>
                                <p class="text-sm text-true-gray-700">
                                    Tambahkan gambar utama yang menarik untuk menggambarkan travel
                                </p>
                            </div>
                            <div class="col-span-1 pt-0 md:pt-4">
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="col-span-1">
                                        <div class="flex items-start space-x-2">
                                            <div class="relative w-full h-32 rounded-md overflow-hidden">
                                                @if ($mainImage)
                                                    @error('mainImage')
                                                        <span class="p-4 w-full h-full flex items-center justify-center bg-true-gray-100 border border-black/75 rounded-md">
                                                            <p class="text-true-gray-700 capitalize text-sm text-center">
                                                                unggah gambar utama
                                                            </p>
                                                        </span>
                                                    @else
                                                        <img 
                                                            src="{{ $mainImage->temporaryUrl() }}" 
                                                            class="w-full h-full object-cover"
                                                        >
                                                    @enderror
                                                @else
                                                    <img 
                                                        src="{{ asset('storage/images/travels/' . $travel->main_image) }}" 
                                                        class="w-full h-full object-cover"
                                                    >
                                                @endif
                                                <div
                                                    wire:loading
                                                    wire:target="mainImage"
                                                    class="absolute w-full h-full inset-0 border border-black/75 rounded-md"
                                                >
                                                    <div class="w-full h-full flex items-center justify-center bg-true-gray-100 rounded-md">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-black animate-spin" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                            <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                                            <path d="M4.05 11a8 8 0 1 1 .5 4m-.5 5v-5h5"></path>
                                                        </svg>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="flex-shrink-0">
                                                <label
                                                    for="hero-image"
                                                    class="block text-black hover:text-true-gray-700 cursor-pointer"
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                                        <path d="M4 20h4l10.5 -10.5a1.5 1.5 0 0 0 -4 -4l-10.5 10.5v4"></path>
                                                        <line x1="13.5" y1="6.5" x2="17.5" y2="10.5"></line>
                                                    </svg>
                                                </label>
                                                <input 
                                                    type="file" 
                                                    wire:model="mainImage" 
                                                    id="hero-image"
                                                    class="hidden"
                                                >
                                            </div>
                                        </div>
                                        @error('mainImage')
                                            <span class="block mt-2 text-xs text-red-600">
                                                {{ $message }}
                                            </span>
                                        @enderror
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-8">
                            <div class="col-span-1">
                                <label 
                                    class="block mb-1 text-black font-medium capitalize"
                                >
                                    destinasi travel
                                </label>
                                <p class="text-sm text-true-gray-700">
                                    Pilih satu atau beberapa destinasi-destinasi travel yang akan ditawarkan
                                </p>
                            </div>
                            <div class="col-span-1 pt-0 md:pt-4">
                                <div class="grid grid-cols-1 gap-4">
                                    @foreach ($destinations as $i => $destination)
                                        <div class="col-span-1">
                                            <div class="flex items-start space-x-3">
                                                <div>
                                                    <select 
                                                        wire:model="destinations.{{ $i }}.travelObjectId"
                                                        class="w-full rounded-lg border-black/75 focus:outline-none focus:ring focus:ring-black/20 focus:border-black"
                                                    >
                                                        <option value=""></option>
                                                        @foreach ($travelObjects as $travelObject)
                                                            <option value="{{ $travelObject->id }}">{{ $travelObject->title }}</option>
                                                        @endforeach
                                                    </select>
                                                    @error('destinations.' . $i . '.travelObjectId')
                                                        <span class="block mt-2 text-xs text-red-600">
                                                            {{ $message }}
                                                        </span>
                                                    @enderror
                                                </div>
                                                <a 
                                                    href="#"
                                                    wire:click.prevent="deleteDestination({{ $i }}, {{ isset($destination['destinationId']) ? $destination['destinationId'] : null }})"
                                                    class="flex-shrink-0 {{ count($destinations) == 1 ? 'text-true-gray-400 cursor-default' : 'text-black hover:text-true-gray-700' }}"
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                                        <rect x="4" y="4" width="16" height="16" rx="2"></rect>
                                                        <path d="M10 10l4 4m0 -4l-4 4"></path>
                                                    </svg>
                                                </a>
                                            </div>
                                        </div>
                                    @endforeach
                                    <div class="col-span-1">
                                        <a 
                                            href="#"
                                            wire:click.prevent="addDestination"
                                            class="inline-flex items-center space-x-2 text-black group"
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                                <rect x="4" y="4" width="16" height="16" rx="2"></rect>
                                                <line x1="9" y1="12" x2="15" y2="12"></line>
                                                <line x1="12" y1="9" x2="12" y2="15"></line>
                                            </svg>
                                            <span class="text-sm capitalize group-hover:underline">
                                                tambah destinasi travel
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-8">
                            <div class="col-span-1">
                                <label 
                                    class="block mb-1 text-black font-medium capitalize"
                                >
                                    catatan
                                </label>
                                <p class="text-sm text-true-gray-700">
                                    Tambahkan beberapa catatan mengenai travel yang nantinya menjadi pedoman untuk konsumen
                                </p>
                            </div>
                            <div class="col-span-1 pt-0 md:pt-4">
                                <div class="grid grid-cols-1 gap-4">
                                    @foreach ($notes as $i => $note)
                                        <div class="col-span-1">
                                            <div class="flex items-start space-x-3">
                                                <div>
                                                    <textarea
                                                        wire:model="notes.{{ $i }}.value"
                                                        class="w-full rounded-lg border-black/75 focus:outline-none focus:ring focus:ring-black/20 focus:border-black"
                                                    ></textarea>
                                                    @error('notes.' . $i . '.value')
                                                        <span class="block mt-0 text-xs text-red-600">
                                                            {{ $message }}
                                                        </span>
                                                    @enderror
                                                </div>
                                                <a 
                                                    href="#"
                                                    wire:click.prevent="deleteNote({{ $i }}, {{ isset($note['noteId']) ? $note['noteId'] : null }})"
                                                    class="flex-shrink-0 {{ count($notes) == 1 ? 'text-true-gray-400 cursor-default' : 'text-black hover:text-true-gray-700' }}"
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                                        <rect x="4" y="4" width="16" height="16" rx="2"></rect>
                                                        <path d="M10 10l4 4m0 -4l-4 4"></path>
                                                    </svg>
                                                </a>
                                            </div>
                                        </div>
                                    @endforeach
                                    <div class="col-span-1">
                                        <a 
                                            href="#"
                                            wire:click.prevent="addNote"
                                            class="inline-flex items-center space-x-2 text-black group"
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                                <rect x="4" y="4" width="16" height="16" rx="2"></rect>
                                                <line x1="9" y1="12" x2="15" y2="12"></line>
                                                <line x1="12" y1="9" x2="12" y2="15"></line>
                                            </svg>
                                            <span class="text-sm capitalize group-hover:underline">
                                                tambah catatan
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-8">
                            <div class="col-span-1">
                                <label 
                                    class="block mb-1 text-black font-medium capitalize"
                                >
                                    fasilitas
                                </label>
                                <p class="text-sm text-true-gray-700">
                                    Tambahkan keterangan beberapa fasilitas yang nantinya akan didapatkan oleh konsumen
                                </p>
                            </div>
                            <div class="col-span-1 pt-0 md:pt-4">
                                <div class="grid grid-cols-1 gap-4">
                                    @foreach ($facilities as $i => $facility)
                                        <div class="col-span-1">
                                            <div class="flex items-start space-x-3">
                                                <div>
                                                    <textarea
                                                        wire:model="facilities.{{ $i }}.value"
                                                        class="w-full rounded-lg border-black/75 focus:outline-none focus:ring focus:ring-black/20 focus:border-black"
                                                    ></textarea>
                                                    @error('facilities.' . $i . '.value')
                                                        <span class="block mt-0 text-xs text-red-600">
                                                            {{ $message }}
                                                        </span>
                                                    @enderror
                                                </div>
                                                <a 
                                                    href="#"
                                                    wire:click.prevent="deleteFacility({{ $i }}, {{ isset($facility['facilityId']) ? $facility['facilityId'] : null }})"
                                                    class="flex-shrink-0 {{ count($facilities) == 1 ? 'text-true-gray-400 cursor-default' : 'text-black hover:text-true-gray-700' }}"
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                                        <rect x="4" y="4" width="16" height="16" rx="2"></rect>
                                                        <path d="M10 10l4 4m0 -4l-4 4"></path>
                                                    </svg>
                                                </a>
                                            </div>
                                        </div>
                                    @endforeach
                                    <div class="col-span-1">
                                        <a 
                                            href="#"
                                            wire:click.prevent="addFacility"
                                            class="inline-flex items-center space-x-2 text-black group"
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                                <rect x="4" y="4" width="16" height="16" rx="2"></rect>
                                                <line x1="9" y1="12" x2="15" y2="12"></line>
                                                <line x1="12" y1="9" x2="12" y2="15"></line>
                                            </svg>
                                            <span class="text-sm capitalize group-hover:underline">
                                                tambah fasilitas
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-8">
                            <div class="col-span-1">
                                <label 
                                    class="block mb-1 text-black font-medium capitalize"
                                >
                                    ketentuan dan kebijakan
                                </label>
                                <p class="text-sm text-true-gray-700">
                                    Tambahkan ketentuan-ketentuan ataupun kebijakan yang harus diketahui oleh konsumen saat menggunakan layanan travel. <br/>
                                    <span class="text-xs font-semibold">
                                        Inputan #1 untuk judul ketentuan atau kebijakan
                                    </span><br/>
                                    <span class="text-xs font-semibold">
                                        Inputan #2 untuk deskripsi dari judul yang diberikan
                                    </span>
                                </p>
                            </div>
                            <div class="col-span-1 pt-0 md:pt-4">
                                <div class="grid grid-cols-1 gap-4">
                                    @foreach ($policies as $i => $policy)
                                        <div class="col-span-1">
                                            <div class="flex items-start space-x-3">
                                                <div class="space-y-2">
                                                    <div>
                                                        <input 
                                                            type="text"
                                                            wire:model="policies.{{ $i }}.title"
                                                            class="w-full rounded-lg border-black/75 focus:outline-none focus:ring focus:ring-black/20 focus:border-black"
                                                        >
                                                        @error('policies.' . $i . '.title')
                                                            <span class="block mt-2 text-xs text-red-600">
                                                                {{ $message }}
                                                            </span>
                                                        @enderror
                                                    </div>
                                                    <div>
                                                        <textarea
                                                            wire:model="policies.{{ $i }}.description"
                                                            class="w-full rounded-lg border-black/75 focus:outline-none focus:ring focus:ring-black/20 focus:border-black"
                                                        ></textarea>
                                                        @error('policies.' . $i . '.description')
                                                            <span class="block mt-0 text-xs text-red-600">
                                                                {{ $message }}
                                                            </span>
                                                        @enderror
                                                    </div>
                                                </div>
                                                <a 
                                                    href="#"
                                                    wire:click.prevent="deletePolicy({{ $i }}, {{ isset($policy['policyId']) ? $policy['policyId'] : null }})"
                                                    class="flex-shrink-0 {{ count($policies) == 1 ? 'text-true-gray-400 cursor-default' : 'text-black hover:text-true-gray-700' }}"
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                                        <rect x="4" y="4" width="16" height="16" rx="2"></rect>
                                                        <path d="M10 10l4 4m0 -4l-4 4"></path>
                                                    </svg>
                                                </a>
                                            </div>
                                        </div>
                                    @endforeach
                                    <div class="col-span-1">
                                        <a 
                                            href="#"
                                            wire:click.prevent="addPolicy"
                                            class="inline-flex items-center space-x-2 text-black group"
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                                <rect x="4" y="4" width="16" height="16" rx="2"></rect>
                                                <line x1="9" y1="12" x2="15" y2="12"></line>
                                                <line x1="12" y1="9" x2="12" y2="15"></line>
                                            </svg>
                                            <span class="text-sm capitalize group-hover:underline">
                                                tambah ketentuan dan kebijakan
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="space-y-6">
                    @if ($validationFailed)
                        <div class="flex items-center space-x-3 px-3 py-2 rounded-md bg-red-100">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 flex-shrink-0" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                <path d="M12 9v2m0 4v.01"></path>
                                <path d="M5 19h14a2 2 0 0 0 1.84 -2.75l-7.1 -12.25a2 2 0 0 0 -3.5 0l-7.1 12.25a2 2 0 0 0 1.75 2.75"></path>
                            </svg>
                            <p class="text-sm font-medium text-black">
                                Pastikan semua inputan sudah diisi dengan lengkap dan benar
                            </p>
                        </div>
                    @endif
                    @if (session()->has('message'))
                        <div class="flex items-center space-x-3 px-3 py-2 rounded-md bg-green-100">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 flex-shrink-0" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                <rect x="4" y="4" width="16" height="16" rx="2"></rect>
                                <path d="M9 12l2 2l4 -4"></path>
                            </svg>
                            <p class="text-sm font-medium text-black">
                                {{ session('message') }}
                            </p>
                        </div>
                    @endif
                    <div class="text-right">
                        <button
                            type="submit"
                            class="px-4 py-2 rounded-lg bg-true-gray-800 hover:bg-true-gray-700 focus:outline-none focus:ring focus:ring-true-gray-700/30 text-white text-base capitalize"
                        >
                            simpan
                        </button>
                    </div>
                </div>
            </form>
        </div>
    </div>
</div>
